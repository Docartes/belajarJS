...
function shoppingTime(memberId, money) {
  ...
  for ( let harga in barang ) {
    while ( changeMoney >= barang[harga] ) {
      changeMoney = changeMoney - barang[harga];
      listPurchased.push(harga);
      for ( let i = 0; i < listPurchased.length; i++ ) {
        if ( listPurchased[i] == listPurchased[i + 1] ) {
          listPurchased = [listPurchased[0]];
        }
      }
      break;
    }
