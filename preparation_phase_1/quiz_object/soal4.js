...
function travelingIndonesia(arr, emoney) {
  ...
  for ( let item of arr ) {
    ...
    for ( let i = 0; i < route.length; i++ ) {
      if ( route[i] == travelData[2] ) {
        tujuan = i
      } else if ( route[i] == travelData[1] ) {
        berangkat = i
      }
    }

    let jarak = Math.abs(tujuan - berangkat)

    if ( travelData[3] == 'Bis' ) {
      totalCost = jarak * 225000;
    } else if ( travelData[3] == 'Kereta' ) {
      totalCost = jarak * 250000;
    } else if ( travelData[3] == 'Pesawat' ) {
      totalCost = jarak * 275000;
    }
    ...
    travelDetail = {
      name: travelData[0],
      departureCity: travelData[1],
      destinationCity: travelData[2],
      transport: travelData[3],
      totalCost: totalCost
    }

    result.push(travelDetail)
  }
