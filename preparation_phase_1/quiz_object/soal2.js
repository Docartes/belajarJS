...
function economyChangeSummary(tradeActivity) {
  ...
  for ( let i = 0; i < tradeActivity.length; i++ ) {
    for ( let j = 0; j < tradeActivity[i].length; j++ ) {
      if ( removeNum(tradeActivity[i][j]) == 'Jeff Bezos') {
        duitJeff = duitJeff + ( duitJeff * (parseInt(removeAlpha(tradeActivity[i][j])) / 100) )
        result.push({
          name: removeNum(tradeActivity[i][j]),
          deposit: duitJeff,
          owner: 'Amazon'
        })
      } else if ( removeNum(tradeActivity[i][j]) == 'Larry Page') {
        duitLarry = duitLarry + ( duitLarry * (parseInt(removeAlpha(tradeActivity[i][j])) / 100) )
        result.push({
          name: removeNum(tradeActivity[i][j]),
          deposit: duitLarry,
          owner: 'Google'
        })
      } else if ( removeNum(tradeActivity[i][j]) == 'Jack Ma') {
        duitJack = duitJack + ( duitJack * (parseInt(removeAlpha(tradeActivity[i][j])) / 100) )
        result.push({
          name: removeNum(tradeActivity[i][j]),
          deposit: duitJack,
          owner: 'Alibaba'
